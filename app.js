...
var mapFile = readFileSync('./maps/maps.json', {encoding:'ascii'});
...
//connect client to master server -- in this version master server does not exist, this game server only communicates with the players browser
...
class LobbyPlayer{
  ...
  hueShift;
  connected;
  constructor(id, name="GuestPlayer", isOwner = false, hueShift=0) {
    this.id = id;
    this.name = name;
...
class ShooterGameLobby{
  ...
  playerJoin(playerId=undefined, name, hueShift=0){
    if(!playerId){
      playerId = THREE.MathUtils.generateUUID();
    }
    var player;
    if(!this.players.has(playerId)){
      player = new LobbyPlayer(playerId, name, (this.players.size==0), hueShift);
      this.players.set(playerId, player);
    }else player = this.players.get(playerId);
    console.log("connected");
    player.connected = true;
    SocketServer.in(this.id).emit("lobbyChanged",{players: Array.from(this.players.values()), maps:maps});
    return player;
  }
  playerLeave(playerId){
    ...
    setTimeout(()=>{
      ...
      if(this.players.get(playerId) && !this.players.get(playerId).connected){
        ...
        SocketServer.in(this.id).emit("lobbyChanged",{players: Array.from(this.players.values())});
      }
    },5*1000)
  }
  gameStart(){
...
SocketServer.on('connection', (socket) => {
  ...
  socket.on("disconnect", (reason) => {
   ...
   console.log(reason);
   if(lobbies.has(socket.handshake.query.lobbyId)){
    lobbies.get(socket.handshake.query.lobbyId).playerLeave(socket.data.playerId);
   }
  });
  ...
  socket.on("playerStateUpdate",(data) => {
    ...
    if(game != undefined){
      var player = game.players.get(socket.data.playerId);
      if(player != undefined){
  ...
  socket.on("pause",(data) => {
    ...
    //console.log(data);
    if(game != undefined){
      if(data.paused)game.pause(game.players.get(socket.data.playerId).nickname+" paused the game.");
      else game.unpause();
    }
  });
  ...
  socket.on("createGame", async (data, callback)=>{
    ...
    //console.log(map);
    games.set(data.id, new ShooterGame(data.id, mapData));
  ...
  socket.on("startGame", (data)=>{
    ...
    try {
      ...
      games.get(socket.handshake.query.lobbyId).setPlayers(players);
      SocketServer.in(socket.handshake.query.lobbyId).emit('loadPlayers');
    } catch (error) {
      console.log(error);
    }
  ...
  socket.on("joinGame", (data, callback)=>{
    socket.join(socket.handshake.query.lobbyId)
    var game = games.get(socket.handshake.query.lobbyId)
    if(game != undefined){
      var player = game.players.get(socket.data.playerId);
      if(player != undefined){
  ...
  /*
    lobby undef player undef - create lobby assing playerid
    lobby undef player def - create lobby 
    lobby def player undef - assign playerid and join lobby if exist - err otherwise
    lobby def player def - join lobby if exist - err otherwise
  */
  socket.on("joinLobby", (data, callback)=>{
    ...
    if(lobbyId){
      if(lobbies.has(lobbyId)){
        ...
        callback({error:true,message:"This lobby does not exist"})
        return;
      }
    }else{
      callback({error:true,message:"Lobby ID not provided"})
      return;
    }
  });
  socket.on("test", ()=>{
...
server.listen(process.env.PORT || PORT || 5000, () => {
  console.log('listening on '+IP+":"+(process.env.PORT || PORT || 5000));
});
